...
app.post('/admin/deposits/:id/approve', adminAuth, async (req, res) => {
    try {
        ...
        // Update user wallet using mobile
        const user = await User.findOne({ mobile: deposit.mobile });
        ...
        console.log(`Deposit ${deposit._id} approved for user ${deposit.mobile}, amount: ₹${deposit.amount}`);
...
app.post('/admin/withdrawals/:id/approve', adminAuth, async (req, res) => {
    try {
        ...
        console.log(`Withdrawal ${withdrawal._id} approved for user ${withdrawal.mobile}, amount: ₹${withdrawal.amount}`);
...
app.post('/admin/withdrawals/:id/reject', adminAuth, async (req, res) => {
    try {
        ...
        // Refund amount to user wallet using mobile
        const user = await User.findOne({ mobile: withdrawal.mobile });
        if (user) {
            user.wallet = Math.round((user.wallet + withdrawal.amount) * 100) / 100;
            await user.save();
        ...
        res.json({ 
            ...
            newBalance: user ? user.wallet : null
        });
    } catch (err) {
...
app.get('/my-tournaments', auth, async (req, res) => {
    try {
        // ✅ FIXED: Get user by mobile first
        const user = await User.findOne({ mobile: req.user.mobile });
        if (!user) {
            return res.status(404).json({ error: 'User not found' });
        }
        ...
        const query = { userId: user._id };  // ✅ Use correct ID
        if (status) query.status = status;
...
app.listen(PORT, () => {
  ...
  console.log(`🎁 Registration Bonus: ₹00 bonus`);
  console.log(`💰 First Deposit Bonus: 20%`);
  console.log(`🎁 Gift Code System: Enabled`);
  console.log(`💰 Referral Levels: 6 (1% total commission)`);
